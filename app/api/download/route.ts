...
async function buildDownloadResponse({ templateId, generationId, liveValues }: DownloadParams) {
  ...
  const valuesRecord = liveValues
    ? sanitizeRecord(liveValues)
    : values.reduce<Record<string, string>>((acc, value) => {
      acc[value.fieldKey] = decryptFieldValue(value.fieldValue);
      return acc;
    }, {});
...
export async function GET(request: NextRequest) {
  try {
    ...
    return await buildDownloadResponse({ templateId, generationId });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const liveValues = typeof body?.values === "object" && body.values !== null
      ? (body.values as Record<string, string>)
      : undefined;

    return await buildDownloadResponse({ templateId, generationId, liveValues });
  } catch (error) {
    unstable_rethrow(error);
    console.error("API Download error:", error);
    return new Response("Internal Server Error", { status: 500 });
